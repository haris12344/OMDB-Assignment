  ...
  return (
    ...
    <div className="flex flex-col md:flex-row gap-3 md:gap-2 justify-between w-full md:w-[1050px] p-5">
      <div className="flex flex-col sm:flex-row gap-2 w-full justify-center">
        <input type="text" ref={movieInput} placeholder="Enter movie name." className="border border-neutral-800 px-4 rounded-xl w-full sm:w-[500px] bg-neutral-800 py-2 focus:outline-2 focus:outline-blue-700"/>
        <button onClick={movieHandler} className="bg-red-600 rounded-xl text-white font-bold px-4 py-2 w-full sm:w-auto">Submit
        </button>
      </div>
      <button className="bg-green-600 px-4 py-2 rounded-lg w-full sm:w-auto">Log-in</button>
    </div>
    </div>
  {
    !movie && !error ? <Intro /> :
